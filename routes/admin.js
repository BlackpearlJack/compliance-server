...
router.get('/user-stats', isAdmin, async (req, res) => {
  try {
    ...
    const [[pendingSubmissions]] = await pool.query('SELECT COUNT(*) as count FROM compliance_forms WHERE status = "pending"');
...
router.post('/update-submission', isAdmin, async (req, res) => {
  ...
  try {
    ...
    const [submission] = await pool.query(
      'SELECT user_id FROM compliance_forms WHERE id = ?',
      [submissionId]
    );
...
router.post('/approve-submission', isAdmin, async (req, res) => {
  ...
  try {
    ...
    const [submission] = await pool.query('SELECT user_id FROM compliance_forms WHERE id = ?', [submissionId]);
    const userId = submission[0]?.user_id;

    if (userId) {
      await pool.query(
        'INSERT INTO notifications (user_id, type, message, related_id) VALUES (?, ?, ?, ?)',
        [userId, 'submission_update', `Your submission (ID: ${submissionId}) has been ${status}.`, submissionId]
      );
    }
